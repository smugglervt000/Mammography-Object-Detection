...
class RetinaNetClassificationHead(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, matched_idxs):
        ...
        for targets_per_image, cls_logits_per_image, matched_idxs_per_image in zip(targets['labels'], cls_logits, matched_idxs):
            ...
            try:
                print(foreground_idxs_per_image.shape, targets_per_image.shape, matched_idxs_per_image.shape)
                gt_classes_target[
                    foreground_idxs_per_image,
                    targets_per_image[matched_idxs_per_image[foreground_idxs_per_image]].long(),
                ] = 1.0
            except:
                print("Failed: ")
                print(foreground_idxs_per_image.shape, targets_per_image.shape, matched_idxs_per_image.shape)
...
class RetinaNetRegressionHead(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, anchors, matched_idxs):
        ...
        for targets_per_image, bbox_regression_per_image, anchors_per_image, matched_idxs_per_image in zip(
            targets['bbox'], bbox_regression, anchors, matched_idxs
        ):
            ...
            matched_gt_boxes_per_image = targets_per_image[matched_idxs_per_image[foreground_idxs_per_image].long()]
...
class RetinaNet(nn.Module):
    ...
    def postprocess_detections(self, head_outputs, anchors, image_shapes):
        ...
        num_images = image_shapes[0] # B, C, H, W
        h = image_shapes[-2]
        w = image_shapes[-1]
        ...
        for index in range(num_images):
            ...
            anchors_per_image, image_shape = anchors[index], (h, w)
    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            if targets is None:
                ...
                boxes = targets["bbox"]
                for box in boxes:
                    torch._assert(isinstance(box, torch.Tensor), "Expected target boxes to be of type Tensor.")
                    torch._assert(
        ...
        if not self.training:
            ...
            detections = self.postprocess_detections(split_head_outputs, split_anchors, images.shape)
...
def retinanet_resnet50_fpn(
    ...
    **kwargs: Any,
    ...
    # custom_weights_path = '/vol/biomedic3/bglocker/mscproj24/mrm123/slurm_scripts/ResNet50/492ytgmx/checkpoints/best_massfil_vindr_2:1.ckpt'
    # custom_checkpoint = torch.load(custom_weights_path)
    # state_dict = custom_checkpoint['state_dict']
    ...
    backbone = resnet50(weights=weights_backbone, progress=progress, norm_layer=norm_layer) # weights_backbone
